...
NAT Reality Bridge 是一个轻量开源自动化部署工具，用于在低资源 NAT VPS 上构建极简 Xray Reality 入口节点，并可选通过 SOCKS5 ISP 或家宽出口完成最终出站。
...
从 v1.1.0 开始，本项目提供交互式安装器、两种部署模式、自动节点 URI 生成、备份脚本和健康检查工具。
...
NAT VPS 入口节点通过服务商端口映射接收客户端流量，并运行极简 Xray-core。在 ISP Residential Exit Mode 中，SOCKS5 outbound 提供最终 ISP 或家宽出口 IP。
...
- 交互式安装器
- Basic Mode：VPS 原生出口
- ISP Residential Exit Mode：SOCKS5 出口
- 自动生成 VLESS URI
- 健康检查和备份辅助脚本
- SOCKS5 outbound
- ISP 家宽出口架构
...
ISP Residential Exit Mode 将所有 Xray 流量路由到带认证的 SOCKS5 ISP 或家宽出口。
...
v1.1.0 安装脚本为交互式。它会先检查 root 权限、Debian 版本、CPU 架构、systemd、内存和磁盘空间，再让用户选择 Basic Mode 或 ISP Residential Exit Mode。

SOCKS5 凭据等敏感参数在运行时输入，不保存在本仓库中。
