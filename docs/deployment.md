...
Run the audit commands above and confirm:
...
Use official Xray-core release assets and verify checksums before installing. Keep files in the standard paths listed above.

### 4. Reality Configuration

Use the Cloudflare Reality profile shown above and generate fresh UUID, keypair, and short ID per node.
...
Run Xray config test, restart through systemd, check listener state, and verify the client exit IP.
